#!/usr/bin/env node
/**
 * DeepSeek Source Extractor - Main Entry
 *
 * Complete pipeline: extract sources from a DeepSeek share page via CDP,
 * then write them to a Feishu Bitable.
 *
 * Usage:
 *   node run.js --url "<deepseek-share-url>" --natural-question NQ-001 \
...
function printHelp() {
  console.log(`DeepSeek Source Extractor - Complete Pipeline

Extract reference sources from a DeepSeek share page and write them to Feishu Bitable.
...
Options:
  --url <url>               DeepSeek share URL (required for extract step)
...
async function main() {
  ...
  if (args.writeOnly) {
    ...
    console.log(`[1/2] Extracting sources from DeepSeek share page...`);
