"""Bridge module: invokes the DeepSeek source extractor JS script.

This module is the integration layer between the Python mobile automation
(DeepSeek 手机端) and the Node.js source extractor (DeepSeek source extractor).

Pipeline:
    手机端捕获 share_url + linked_natural_question
        -> 本桥接模块校验参数
        -> 调用 node deepseek-source-extractor/run.js
        -> JS 脚本通过 CDP 在分享页上下文调用 share/content API、提取来源、写回飞书
        -> 返回结构化结果供 Python 端记录与回写跟踪

设计要点：
    * 参数校验：share_url 必须为 DeepSeek 分享链接，natural_question 不得为空
...
from .constants import DEEPSEEK_SHARE_URL_RE_PATTERN
from .time_utils import now_iso, stamp


# 默认配置：与项目根目录的 deepseek-source-extractor/run.js 对齐
DEFAULT_SCRIPT_RELATIVE = Path("deepseek-source-extractor", "run.js")
DEFAULT_CDP_URL = "http://127.0.0.1:9222"
DEFAULT_TIMEOUT_SECONDS = 120
DEFAULT_MAX_RETRIES = 2
DEFAULT_RETRY_BACKOFF_BASE = 2.0  # 指数退避基数（秒）

# DeepSeek 分享链接校验（U2 实测）：https://chat.deepseek.com/share/<id>
DEEPSEEK_SHARE_URL_RE = re.compile(DEEPSEEK_SHARE_URL_RE_PATTERN, re.IGNORECASE)
...
class ExtractorOptions:
    ...
    script_path: str = ""  # 留空则自动定位到项目根的 deepseek-source-extractor/run.js
...
def resolve_script_path(explicit: str = "") -> Path:
    ...
    raise SourceExtractorError(
        f"Could not locate deepseek-source-extractor/run.js. "
...
def validate_params(share_url: str, natural_question: str) -> None:
    ...
    if not DEEPSEEK_SHARE_URL_RE.match(share_url.strip()):
        raise SourceExtractorError(
            f"share_url does not look like a DeepSeek share URL: {share_url[:80]!r}. "
            f"Expected: https://chat.deepseek.com/share/<id>"
        )
...
def _get_logger(output_dir: str | Path) -> logging.Logger:
    ...
    logger = logging.getLogger(f"deepseek.source_extractor.{stamp()}")
    if logger.handlers:
        return logger
...
def run_source_extractor(
    ...
    When *base_token* is empty the extractor runs in extract-only mode: it
    fetches content (answer, thinking, sources) from the share page API but
    does **not** write to Feishu. A *table_id* alone cannot drive a writeback
    (the lark-cli call needs an app token), so base_token presence is the sole
    signal that distinguishes full pipeline from extract-only.
    """
