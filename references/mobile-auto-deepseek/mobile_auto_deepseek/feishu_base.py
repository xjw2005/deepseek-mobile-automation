...
FEISHU_ANSWER_TABLE_ID = "tblz1qWt0EmduKwD"
FEISHU_SOURCE_TABLE_ID = "tbltRcXROHY3NUdE"
...
def clean_answer_for_writeback(value) -> str:
    """Trim answer preludes, reference markers, and normalize the final answer text."""
    text = clean_text(value)
    if not text:
        return ""
...
def run_json_command(command: str, args: list[str]) -> dict:
    ...
    # lark-cli dry-run payloads describe the planned API calls and omit `ok`.
    if proc.returncode != 0 or (not payload.get("ok") and "api" not in payload):
...
def run_json_command_with_payload(command: str, args: list[str], payload: dict) -> dict:
    ...
    try:
        ...
        temp_path = payload_dir / f"deepseek-{stamp()}.json"
...
def build_task_from_feishu(args) -> dict:
    ...
    task = {
        "taskName": "deepseek-feishu-base-run",
        "mode": "separate",
        ...
        "output": f"results/deepseek-feishu-base-run-{stamp()}.json",
    }
...
def planned_writeback(task: dict, enabled: bool, mark_collected: bool = False, answer_table_id: str = "", source_table_id: str = "") -> dict:
    ...
    return {
        ...
        "action": "create DeepSeek answer records; source records written by JS extractor" if extractor_enabled else "create DeepSeek answer records only; source writeback is disabled",
        ...
        "sourceWriter": "deepseek-source-extractor (JS)" if extractor_enabled else None,
    }
