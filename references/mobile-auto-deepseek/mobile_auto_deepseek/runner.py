...
from .device_lock import DeviceProcessLock
...
def thinking_force_requested(value: object) -> bool:
    """Only a literal boolean true authorizes changing DeepSeek's toggle."""
    return value is True


def _setup_run_logger(output_path: str) -> logging.Logger:
    """为整次任务创建一个 run logger，写入 output 同级目录。"""
    logger = logging.getLogger("deepseek.runner")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--link-only", action="store_true",
                        help="Skip mobile-side thinking/answer capture; rely on the JS extractor (--extract-sources) to fetch content from the share page API.")
    parser.add_argument("--debug", action="store_true")
    # —— JS 来源提取器集成 ——
    parser.add_argument(
        "--extract-sources",
        action="store_true",
        help="After capturing the answer share link, invoke the deepseek-source-extractor JS script to extract sources and write them to Feishu.",
    )
    parser.add_argument("--cdp-url", default="http://127.0.0.1:9222", help="Chrome DevTools Protocol endpoint for the JS extractor.")
    parser.add_argument("--extractor-script", default="", help="Path to deepseek-source-extractor/run.js. Auto-located if omitted.")
...
def run_question(
    ...
    try:
        ...
        # Tri-state semantics: only an explicit JSON/Feishu boolean true may
        # touch the DeepSeek toggle. Missing, null, and false preserve the
        # app's current mode.
        requested_thinking = thinking_force_requested(question_item.get("thinking"))
        _ts = time.monotonic()
        thinking = enter_thinking_mode(adb, output_dir) if requested_thinking else {
            "requested": question_item.get("thinking"),
            "changed": False,
            "verified": True,
            "reason": "left_unchanged",
        }
        ...
        _ts = time.monotonic()
        answer_result = wait_for_answer(adb, question, output_dir, timeout=180.0, stable_seconds=int(task["options"].get("waitStableSeconds", 2)))
        ...
        try:
            if requested_thinking and not link_only:
                ...
                thinking_capture = capture_answer_page_thinking_content(
                    ...
                    output_dir,
                    question=question,
                    max_scrolls=int(task["options"].get("expertAnswerMaxScrolls", 12)),
        ...
        # 传递给 deepseek-source-extractor JS 脚本，由其访问分享页、提取来源、写回飞书。
        sources: list[dict] = []
        ...
        if extractor_enabled and share_url:
            ...
            try:
                ...
                # share/content API is authoritative and is not limited to the
                # currently visible mobile viewport. Prefer its complete answer
                # and reasoning after the share link has been created.
...
def _run_task_locked(task: dict, adb: AdbClient, writeback_context: dict | None = None, source_extractor_context: dict | None = None) -> str:
    """Execute a task after the caller has exclusively locked its device."""
    run_logger = _setup_run_logger(task["output"])
    run_logger.info("=== Task started: %s ===", task.get("taskName", "deepseek-mobile-run"))
    ...
    aggregate = {
        "taskName": task.get("taskName", "deepseek-mobile-run"),
    ...
    run_logger.info("=== Task finished: %s ===", task.get("taskName", "deepseek-mobile-run"))
    return output


def run_task(task: dict, writeback_context: dict | None = None, source_extractor_context: dict | None = None) -> str:
    """Resolve and exclusively lock the target device for the complete run."""
    device = task["device"]
    adb = AdbClient(device.get("adb"), device.get("serial"))
    serial = adb.resolve_serial()
    with DeviceProcessLock(serial):
        return _run_task_locked(task, adb, writeback_context, source_extractor_context)
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.task and (args.base_url or args.table_id):
        raise ValueError("With --task, use --base-token only for output writeback; input table flags are not allowed.")
    ...
    if args.link_only and not args.extract_sources:
        raise ValueError(
            ...
            "so answer/thinking content must come from the JS extractor (share page API)."
        )


def force_quick_mode(task: dict) -> None:
    """Prevent the runner from force-enabling thinking for this task."""
    task["thinking"] = False
